...
def matchset(pattern, text):
    ...
    #print pattern, text, op, x, y
    if 'lit' == op:
        return set([text[len(x):]]) if text.startswith(x) else null
    elif 'seq' == op:
        #print 26, matchset(x, text)
        return set(t2 for t1 in matchset(x, text) for t2 in matchset(y, t1))
    elif 'alt' == op:
        return matchset(x, text) | matchset(y, text)
    elif 'dot' == op:
        #print 31, set(''.join(text[1:]))
        #print 31, text, set(''.join(text[1:]))
        res = set([text[1:]])
        #print 33, res, set(res)
        return res
    elif 'oneof' == op:
        return set([text[1:] for a in x if text[0] == a])
    elif 'eol' == op:
