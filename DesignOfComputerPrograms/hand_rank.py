...
def hand_rank(hand):
    ...
    if straight(ranks) and flush(hand):            # straight flush
        ...
        return (5, sorted(hand, reverse = True))
    elif straight(ranks):                          # straight
        return (4, max(hand))
    elif kind(3, ranks):                           # 3 of a kind
        return (3, kind(3, ranks), sorted(hand, reverse = True))
    elif two_pair(ranks):                          # 2 pair
        return (2, two_pair(ranks)[0], two_pair(ranks)[1], sorted(hand, reverse=True))
    elif kind(2, ranks):                           # kind
        return (1, kind(2, ranks), sorted(hand, reverse=True))
    else:                                          # high card
        return (0, sorted(hand, reverse=True))
