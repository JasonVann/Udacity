...
def csuccessors(state):
    ...
    if B1 > 0:
        ...
        if M1 >= 2:
            dic[(M1-2, C1, B1-1, M2+2, C2, B2+1)] = 'MM->'
        if M1 >= 1:
            dic[(M1-1, C1, B1-1, M2+1, C2, B2+1)] = 'M->'
        
        if M1 >= 1 and C1 >= 1:
            dic[(M1-1, C1-1, B1-1, M2+1, C2+1, B2+1)] = 'MC->'
            
        if C1 >= 2:
            dic[(M1, C1-2, B1-1, M2, C2+2, B2+1)] = 'CC->'
        if C1 >= 1:
            dic[(M1, C1-1, B1-1, M2, C2+1, B2+1)] = 'C->'
    if B2 > 0:
